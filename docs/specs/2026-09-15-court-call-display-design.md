...
**A pure read-only Compose projection over the existing `session` `StateFlow`.** `CourtCallScreen` collects `viewModel.session` (the same flow the Hub reads) and renders `session.courts`, each court's `currentMatch`, and `session.activeRecommendations`. It computes a **derived per-court display state** (§5) at composition time — no state is stored, so the board can never drift from the phone. This is the "strip to the public-display minimum" resolution the context modifier prescribes, and it keeps the diff to two new UI files plus nav wiring, proportionate to the feature.
...
- **Entry point:** a **"Court Call"** action in the **Session Hub top bar**, enabled **only when a session exists** (`session != null`); disabled/hidden otherwise, since the board would have nothing to project.
...
- **Display hygiene (from `MainActivity`):** while `CourtCall` is the current screen, set `WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON` and enter **immersive full-screen** (hide the status + navigation bars via `WindowInsetsControllerCompat`). **Clear both on exit** (restore bars, clear the keep-on flag) so the rest of the app is unaffected. Drive this off the collected `currentScreen` in `onCreate`'s `setContent` (an effect keyed on `screen is AppScreen.CourtCall`), and mirror it in the Activity lifecycle so backgrounding also clears it.
...
| **FINAL** | `currentMatch != null` **and** `currentMatch.isCompleted == true` (derived — there is **no** `FINAL` `CourtStatus`) | Muted tile, `"FINAL 11–7"` (final score) |
...
- **Auto-cycle ONLY on overflow.** The per-page capacity is a **responsive function of available size**, not a magic number: the grid fits as many tiles as keep each tile above the §6 legibility floor for the current viewport (e.g. up to a 2×2 on a smaller mirror, up to a 2×3 on a large TV — derive columns/rows from available width/height and the minimum legible tile size).
...
- **Pagination + stability** — pagination controls appear **only** when `courtCount > capacity`; court **ordering is stable and positions are fixed** across renders / page turns (a given `courtId` maps to the same slot).
- **Keep-awake lifecycle** — `FLAG_KEEP_SCREEN_ON` is set on entering `CourtCall` and **cleared on exit** (and on backgrounding).
- **Entry gating** — the Hub's "Court Call" action is **disabled when `session == null`** and enabled when a session exists.
