...
- `SessionViewModel.createInitialSession()` (`SessionViewModel.kt:67`) stops seeding a fake demo session; first launch starts with **no active session**. This is the narrowed ViewModel exception in §2 and the real fix for the recognition finding (the Setup form defaults in finding #7 were only the visible half).
- **Companion Hub empty state (required — chosen over changing the initial route).** `_currentScreen` stays defaulted to `SessionHub`, and `SessionHubScreen` today does `val activeSession = session ?: return` (`SessionHubScreen.kt:40`) — which, once seeding is removed, would render a **blank dead-end with no Scaffold and no way to reach Setup**. Replace that early-return with a real empty state: a Scaffold showing "No active session" + a primary `Start a session` CTA that navigates to Setup (`viewModel.navigateTo(AppScreen.Setup)`). This is a **Compose UI change, not a routing/VM change**, so the §2 ViewModel exception stays limited to `createInitialSession`. It also correctly handles the "all sessions ended" case, not just first launch.
...
- **Screenshot baselines (Roborazzi — already configured):** the repo already wires Roborazzi (`app/build.gradle.kts`: `roborazzi` plugin, `roborazzi.compose`, `roborazzi.junit.rule`) and has a working example test at `app/src/test/java/com/example/GreetingScreenshotTest.kt`. Follow its Roborazzi mechanics to capture key screens (Hub, Live Scoreboard, Setup, both sheets) in **both dark and light**, verified in CI. No new screenshot dependency is needed. (`GreetingScreenshotTest` renders `PickleballAppContent` off the live VM and captures `app_home.png`; the committed `greeting.png` is a stale/orphaned file, **not** a pattern to copy.)
...
**Baseline housekeeping & `GreetingScreenshotTest`:** `GreetingScreenshotTest.kt:39` captures `app_home.png`, while the committed `greeting.png` is orphaned — remove the stale file when regenerating and document the Roborazzi record-vs-verify workflow so CI baseline churn is intentional. Because it renders `PickleballAppContent` off the live VM (`GreetingScreenshotTest.kt:28-31`), after the seeding removal it would capture the blank Hub — so this test must be **rewritten to inject a fixed fixture (or removed)** as part of the deterministic-capture change above, not left silently producing an empty baseline.
